...
int main(int argc, char *argv[]) {
    ...
    // Dividir el espacio de claves en bloques
    long block_size = upper_limit / size;
    long block_start = rank * block_size;
    long block_end = (rank == size - 1) ? upper_limit : (block_start + block_size);

    for (long key = block_start; key < block_end && !found; key++) {
